...
out/views/*.mmd
   │
   ▼ Lucid MCP (or fallback: draw.io MCP / paste Mermaid into Lucid)
...
# then open this directory in Claude Code and run:
...
| Render           | Lucid MCP (`mcp.lucid.app`)   | Native editable shapes in Lucidchart; OAuth, no API key juggling     |
| Fallback         | draw.io MCP (jgraph official) | 10k+ shape libraries; fully editable .drawio                         |
